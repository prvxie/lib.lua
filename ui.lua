...
do
	...
	local shortened_characters = {
		...
		[Enum.UserInputType.MouseButton3] = "m3",[Enum.UserInputType.MouseButton4] = "m4",[Enum.UserInputType.MouseButton5] = "m5",
...
function Library:Window(options)
    local win = menu:window({
        ...
        description = "private build",
        size = vector2_new(575, 450),
        flags = {
            name = options.Name or "menu",
            version = "live"
        }
    ...
    function windowAdapter:Page(pageArgs)
        local tabName = pageArgs.Name or "Tab"
        local iconName = "menu"
        if tabName:lower() == "aimbot" or tabName:lower() == "silent" then iconName = "target"
        elseif tabName:lower() == "world" or tabName:lower() == "visuals" then iconName = "visuals"
        elseif tabName:lower() == "misc" or tabName:lower() == "players" then iconName = "misc" end

        local tab = win:tab({ name = tabName, icon = iconName })
        local pageAdapter = {}
        
        function pageAdapter:MultiSection(multiArgs)
            local multiAdapter = {}
            local side = multiArgs.Side == 1 and "left" or "right"
            
            function multiAdapter:Add(sectionName)
                local group = tab:group({ name = sectionName, side = side })
                local sectionAdapter = {}
                
                function sectionAdapter:Toggle(tArgs)
                    local flag = tArgs.Flag
                    local tObj = group:toggle({ 
                        name = tArgs.Name, 
                        flag = flag, 
                        default = tArgs.Default, 
                    ...
                    function toggleAdapter:SetVisibility(visible)
                        tObj:set_visible(visible)
                    end
                    function toggleAdapter:Set(val)
                        tObj:set_toggle(val)
                    end
                ...
                function sectionAdapter:Slider(sArgs)
                    ...
                    local sObj = group:slider({
                        name = sArgs.Name,
                        flag = flag,
                        min = sArgs.Min,
                        max = sArgs.Max,
                        default = sArgs.Default,
                    ...
                    Library.Flags[flag] = sArgs.Default or sArgs.Min
                    
                    local sliderAdapter = {}
                    function sliderAdapter:SetVisibility(v)
                        sObj:set_visible(v)
                    end
                    function sliderAdapter:Set(v)
                        sObj:set_slider(v)
                    end
                ...
                function sectionAdapter:Dropdown(dArgs)
                    ...
                    local ddAdapter = {}
                    function ddAdapter:SetVisibility(v)
                        dObj:set_visible(v)
                    end
                    function ddAdapter:Set(v)
                        dObj:set_dropdown(v)
                    end
                    function ddAdapter:Refresh(items)
                        dObj:update_dropdown(items)
                    end
                    
                    Library.SetFlags[flag] = function(val)
                        dObj:set_dropdown(val)
                    end
                    return ddAdapter
                end
                ...
                function sectionAdapter:Label(textArgs)
                    ...
                    local lObj = group:label({ name = name })
                    return lObj
                end
...
function Library:Watermark(args)
    -- dd.txt likely has a watermark or we can just ignore or map it to something
end
