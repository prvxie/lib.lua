        ...
        Library.CreateKeybind = function(Self, Data)
            ...
            function Keybind:Set(Key) -- this is so shit but its whatever
                ...
                elseif type(Key) == "table" then
                    local RealKey = (Key.Key == "Backspace" or Key.Key == "Unknown" or tostring(Key.Key):find("Unknown")) and "none" or Key.Key
                    Keybind.Key = tostring(Key.Key)
                    
                    if Keybind.Key == "none" or Keybind.Key:find("Unknown") then
                        Keybind.Key = "Enum.KeyCode.Unknown"
                        Keybind.Value = "none"
                        Items["KeyButton"].Instance.Text = "none"
                        if Key.Mode then
            ...
            Library:Connect(UserInputService.InputBegan, function(Input, GPE)
                if Keybind.Value == "none" or Keybind.Value == "None" then
                    return
                end
            ...
            function MultiSection:Add(Name)
                ...
                local SubItems = { } do
                    SubItems["Button"] = Library:Create("TextButton", {
                        ...
                        BorderSizePixel = 0,
                        AutomaticSize = Enum.AutomaticSize.X,
                        Parent = MultiSection.Items["TabButtons"].Instance
                    }):AddToTheme({TextColor3 = 'Inactive Text', BackgroundColor3 = 'Background'})
                    ...
                    SubItems["Content"] = Library:Create("Frame", {
                        Name = "\0",
                        Parent = MultiSection.Items["Content"].Instance,
                        BackgroundTransparency = 1,
                        Size = UDim2.new(1, 0, 0, 0),
                        BorderSizePixel = 0,
                        Visible = false,
                    ...
                    SubTab.Items = SubItems
                end

                SubItems["Button"]:OnHover(function()
                    if SubTab.Active then return end
                    SubItems["Button"]:Tween({TextColor3 = Library.Theme.Text})
                end, function()
                    if SubTab.Active then return end
                    SubItems["Button"]:Tween({TextColor3 = Library.Theme["Inactive Text"]})
                end)

                function SubTab:Turn()
                    local Old = MultiSection.CurrentTab
                    if Old == SubTab then return end

                    if Old then
                        Old.Items["Content"].Instance.Visible = false
                        Old.Items["Button"]:ChangeItemTheme({TextColor3 = "Inactive Text"})
                        Old.Items["Button"]:Tween({TextColor3 = Library.Theme["Inactive Text"]})
                        Old.Active = false
                    end

                    SubItems["Content"].Instance.Visible = true
                    SubItems["Button"]:ChangeItemTheme({TextColor3 = "Accent"})
                    SubItems["Button"]:Tween({TextColor3 = Library.Theme.Accent})
                    SubTab.Active = true
                    MultiSection.CurrentTab = SubTab
                end
                ...
                if #MultiSection.SubTabs == 0 then
                    SubTab:Turn()
                end
